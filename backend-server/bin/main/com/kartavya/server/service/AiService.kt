...
class AiService(
    ...
    fun processComplaint(request: AiProcessRequest): AiProcessResponse {
        ...
        val aiResponse = if (geminiApiKey.isNotBlank()) {
            try {
                val geminiResponse = callGeminiVision(request, transcript)
                if (geminiResponse != null) {
                    geminiResponse
                } else {
                    fallbackCivicAnalysis(request, transcript)
                }
            } catch (e: Exception) {
        ...
        return aiResponse.copy(
            firestore = FirestoreResult(
                written = firebaseService.saveComplaint(request, aiResponse),
    ...
    private fun transcribeAudio(audioUrl: String): String {
        ...
        try {
            val audio = mediaStorage.read(audioUrl, "audio")

            logger.info("Calling Sarvam STT API for ${audio.fileName} (${audio.bytes.size} bytes)")
            ...
            conn.outputStream.use { os ->
                ...
                // Field: file
                os.write("--$boundary$lineBreak".toByteArray(StandardCharsets.UTF_8))
                os.write("Content-Disposition: form-data; name=\"file\"; filename=\"${audio.fileName}\"$lineBreak".toByteArray(StandardCharsets.UTF_8))
                os.write("Content-Type: ${audio.contentType}$lineBreak$lineBreak".toByteArray(StandardCharsets.UTF_8))
    ...
    private fun callGeminiVision(request: AiProcessRequest, transcript: String): AiProcessResponse? {
        val image = mediaStorage.read(request.imageUrl, "images")
